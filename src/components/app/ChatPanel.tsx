...
import {
  ...
  PanelLeftClose, PanelLeftOpen, Radio, Search, SkipForward, Trash2, Volume2, X,
} from "lucide-react";
...
/* ---------- блок быстрого управления озвучкой ---------- */
function TtsQuickBar({
  speech,
  onToggle,
  toast,
  compact = false,
    ...
    return (
      ...
      >
        <button
          type="button"
          onClick={() => {
            onToggle(!speech.enabled);
            toast(speech.enabled ? "Озвучка выключена" : "Озвучка включена");
          }}
          title={speech.enabled ? "Выключить озвучку" : "Включить озвучку"}
          className="grid h-8 w-8 place-items-center rounded-xl border transition-colors"
          style={{
            borderColor: speech.enabled ? "#8b5cf6" : "var(--dw-line)",
            background: speech.enabled ? "#8b5cf6" : "var(--dw-input)",
        ...
        <button
          ...
          className="grid h-8 w-8 place-items-center rounded-xl border transition-colors disabled:opacity-40"
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1">
      ...
      >
        ...
        {/* блок озвучки — под списком каналов (desktop) */}
        <div className="border-t" style={{ borderColor: "var(--dw-line)" }}>
          <TtsQuickBar
            speech={speech}
            onToggle={onSpeechEnabledChange}
            toast={toast}
      ...
      <div className="flex min-w-0 flex-1 flex-col" style={{ background: "var(--dw-bg)" }}>
        ...
        {/* панель озвучки — только на мобильных (на desktop находится в панели каналов) */}
        <div
          className="shrink-0 border-t md:hidden"
          style={{ borderColor: "var(--dw-line)", background: "var(--dw-panel)" }}
        >
          <TtsQuickBar speech={speech} onToggle={onSpeechEnabledChange} toast={toast} />
        </div>
      </div>
    </div>
  );
