...
import { useSetting } from "../../lib/bridge";
...
export default function ChatPanel({
  ...
  /* показывать служебные события (подключение/ошибка канала) — переключается колокольчиком */
  const [showEvents, setShowEvents] = useSetting<boolean>("showEvents", true);
      ...
      <div className="flex min-w-0 flex-1 flex-col" style={{ background: "var(--dw-bg)" }}>
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              type="button"
              title={showEvents ? "Показывать события подключения" : "Скрыть события подключения"}
              onClick={() => setShowEvents(!showEvents)}
              className="grid h-8 w-8 place-items-center rounded-full border transition-colors"
              style={{
                borderColor: showEvents ? "#22d3ee55" : "var(--dw-line)",
                background: showEvents ? "#22d3ee18" : "var(--dw-input)",
                color: showEvents ? "#22d3ee" : "var(--dw-dim)",
              }}
            >
              {showEvents ? <Bell size={13} /> : <BellOff size={13} />}
            </button>
            <button
              type="button"
            ...
            {shown.map((m) =>
              m.sys ? (
                <div key={m.id} className="flex items-center gap-3 py-2.5">
                  ...
                  <span className="inline-flex items-center gap-1.5 font-mono text-[10.5px]" style={{ color: "var(--dw-dim)" }}>
                    <span
                      className="h-1.5 w-1.5 rounded-full"
                      style={{ background: "var(--dw-dim)", opacity: 0.6 }}
                    />
                    {m.text}
                  </span>
                  <span className="h-px flex-1" style={{ background: "var(--dw-line)" }} />
