...
import { getMessageEffectClass } from "../../lib/message-motion";
...
export default function ChatPanel({
  ...
  /* смайлы площадок: глобальные + пользовательские наборы каналов.
     version меняется после загрузки наборов → лента перерисуется с картинками. */
  const emoteVersion = useEmotes(channels);
  void emoteVersion;
          ...
          <div className="space-y-[3px]">
            {shown.map((m) =>
              m.sys ? (
                ...
                <div
                  key={m.id}
                  className={`group flex items-start gap-3 px-3.5 py-2.5 transition-colors hover:bg-[var(--dw-hover)] ${getMessageEffectClass(viewCfg.messageEffect)}`}
                  style={{
                    borderRadius: Math.max(viewCfg.radius, 14),
                    marginBottom: viewCfg.rowGap,
                    ["--msg-dur" as string]: `${viewCfg.effectDuration ?? 0.34}s`,
                    background:
                      viewCfg.style === "glass"
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    >
                      {parseEmotes(m.text, m.emotes).map((part, i) =>
                        part.type === "emote" ? (
                          <img
                            key={`${m.id}-e${i}`}
                            src={part.url}
                            ...
                            style={{ height: Math.max(18, viewCfg.fontSize * 1.45) }}
                            onError={(e) => {
                              // если CDN недоступен — показываем имя эмодзи текстом
                              const el = e.currentTarget;
                              el.replaceWith(document.createTextNode(part.value));
                            }}
                          />
                        ) : (
