...
import { ChevronDown, Eye, ListX, Mic, Pause, Play, SkipForward, Volume2 } from "lucide-react";
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          <Toggle
            ...
            hint="Когда включено, звук идёт из Browser Source в OBS. Включи в OBS 'Управлять аудио через OBS' — тогда зрители услышат озвучку."
            on={cfg.obsTts}
            onChange={(v) => {
              speech.setObsTts(v);
              onChange({ obsTts: v });
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => {
                  speech.setBackend("sapi");
                  onChange({ backend: "sapi" });
                }}
                className="rounded-lg border px-3 py-2 text-[11.5px] font-semibold"
                style={{
                  borderColor: cfg.backend === "sapi" ? "#8b5cf6" : "var(--dw-line)",
                  background: cfg.backend === "sapi" ? "#8b5cf61a" : "var(--dw-input)",
                  color: cfg.backend === "sapi" ? "#a78bfa" : "var(--dw-dim)",
                }}
              >
                Windows SAPI
              </button>
              <button
                onClick={() => {
                  speech.setBackend("silero");
                  onChange({ backend: "silero" });
                }}
                className="rounded-lg border px-3 py-2 text-[11.5px] font-semibold"
                style={{
                  borderColor: cfg.backend === "silero" ? "#22d3ee" : "var(--dw-line)",
                  background: cfg.backend === "silero" ? "#22d3ee1a" : "var(--dw-input)",
                  color: cfg.backend === "silero" ? "#22d3ee" : "var(--dw-dim)",
                }}
              >
                Silero RU
              </button>
            ...
            {cfg.backend === "silero" && (
              <p className="mt-2 text-[10px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                Silero — нейросетевые русские голоса (см. SILERO_GUIDE.md). Требует Python + torch.
                Если не установлено — автоматически используется SAPI.
              </p>
            )}
          </div>
          ...
          {cfg.backend === "silero" ? (
            <div>
              ...
              <div className="relative">
                <select
                  value={cfg.sileroSpeaker}
                  onChange={(e) => {
                    speech.setSileroSpeaker(e.target.value);
                    onChange({ sileroSpeaker: e.target.value });
                  }}
                ...
                >
                  ...
                  <option value="kseniya">kseniya (женский, по умолчанию)</option>
