...
export default function VoicePanel({
  ...
  const isRu = (v: SpeechSynthesisVoice) => {
    ...
    if (lang === "419" || lang === "1049" || lang === "0x419") return true;
    return /ru|рус|silero|irina|pavel|ирина|павел|aidar|baya|kseniya|xenia|eugene/.test(name);
  };

  const sortedVoices = useMemo(() => {
    const all = [...speech.voices].sort((a, b) => {
      const ar = isRu(a) ? 0 : 1;
      const br = isRu(b) ? 0 : 1;
      if (ar !== br) return ar - br;
      return a.name.localeCompare(b.name, "ru");
    });
    if (!ruOnly) return all;
    const ru = all.filter(isRu);
    return ru.length ? ru : all;
  }, [speech.voices, ruOnly]);
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                onClick={() => {
                  speech.reloadVoices();
                  toast("Обновляю список голосов…");
                }}
                className="rounded-md px-2 py-1 font-mono text-[10px] hover:underline"
            ...
            <div className="mt-1.5">
              <Toggle
                label="Только русские голоса"
                hint="скрывает английские голоса Windows, чтобы случайно не выбрать их"
                on={ruOnly}
                onChange={setRuOnly}
              />
            </div>
            {desktop && (
              <div className="mt-1.5 space-y-1.5">
                <p className="text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                  ...
                  Если выбранный голос недоступен, приложение автоматически возьмёт русский.
                </p>
                <p className="text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                  <span className="font-semibold">Silero SAPI5:</span> после установки пакета Silero нажмите «Обновить».
                  Голоса появятся как «Silero - aidar», «Silero - xenia» и т.д. Инструкция — файл SILERO_VOICES.md в репозитории.
                  Только русский язык.
                </p>
              </div>
            )}
