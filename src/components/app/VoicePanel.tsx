...
import { Bot, ChevronDown, Download, Eye, ListX, Loader, Mic, Pause, Play, SkipForward, Volume2 } from "lucide-react";
                  ...
                  <optgroup label="Silero — нейросеть, только русский (встроенные)">
                    {sileroSpeakers.map((s) => (
                      <option key={`silero:${s}`} value={`silero:${s}`}>
                        {SILOERO_LABELS[s] || `Silero: ${s} (встроенный)`}
                      </option>
                    ))}
                  </optgroup>
                )}
