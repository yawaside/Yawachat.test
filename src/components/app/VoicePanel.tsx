...
export default function VoicePanel({ speech, cfg, onChange, toast }: {
  ...
  const sortedVoices = useMemo(
    () => [...speech.voices].sort((a, b) => {
      const ar = /^ru/i.test(a.lang || "") || /ru|рус|russian|ирина|павел/i.test(a.name) ? 0 : 1;
      const br = /^ru/i.test(b.lang || "") || /ru|рус|russian|ирина|павел/i.test(b.name) ? 0 : 1;
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          ...
          <div>
            ...
            <p className="mt-1.5 text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>Голоса берутся из Windows: Параметры → Время и язык → Речь → Управление голосами.</p>
          </div>
        </div>
      </Panel>
