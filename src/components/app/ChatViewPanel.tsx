    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-2">
      ...
      >
        <div className="space-y-2 overflow-hidden rounded-xl border p-3" style={{ borderColor: "var(--dw-line)", background: "var(--dw-bg)" }}>
          {demo.map((m, index) => (
            <div
              key={`${m.id}-${previewRun}-${cfg.messageEffect}`}
              className={`flex items-start gap-3 px-2 py-2 ${getMessageEffectClass(cfg.messageEffect)}`}
              style={{
                borderRadius: cfg.radius,
                marginBottom: cfg.rowGap,
                animationDelay: `${index * 0.08}s`,
                ["--msg-dur" as string]: `${cfg.effectDuration}s`,
                background:
                  cfg.style === "glass"
