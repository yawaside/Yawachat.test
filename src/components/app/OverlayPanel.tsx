...
function ChipBtn({
  on, onClick, icon, onLabel, offLabel,
}: {
  on: boolean;
  onClick: () => void;
  icon: ReactNode;
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-left font-mono text-[11px] transition-all hover:border-viol"
      style={{
        borderColor: on ? "#8b5cf6" : "var(--dw-line)",
        background: on ? "#8b5cf61a" : "var(--dw-input)",
      ...
      <div className="min-w-0 space-y-4">
        ...
        >
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <ChipBtn
              ...
              onLabel="ловит мышь"
              offLabel="клики насквозь"
            />
            <ChipBtn
              on={!cfg.locked}
              onClick={() => set({ locked: !cfg.locked })}
              icon={cfg.locked ? <Lock size={12} /> : <Unlock size={12} />}
              onLabel="позиция свободная"
              offLabel="позиция зафиксирована"
            />
          </div>
        </Panel>
        ...
        <Panel title="Горячие клавиши оверлея">
          <div className="space-y-2">
            {[
              ["Показать / скрыть оверлей", ["Ctrl", "Shift", "G"]],
              ["Сквозные клики", ["Ctrl", "Shift", "C"]],
            ].map(([label, keys]) => (
              <div
                key={String(label)}
              ...
              >
                <span className="min-w-0 text-[12px]">{label as string}</span>
                <span className="flex shrink-0 flex-wrap items-center gap-1">
                  {(keys as string[]).map((k, i) => (
                    <span key={k} className="inline-flex items-center gap-1">
                      {i > 0 && <span className="text-[10px]" style={{ color: "var(--dw-dim)" }}>+</span>}
            ...
            <p className="text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
              Комбинации меняются на вкладке «Горячие клавиши».
            </p>
          </div>
        </Panel>
      </div>
