...
import {
  DEFAULT_CHAT_VIEW, DEFAULT_HOTKEYS, DEFAULT_OVERLAY, DEFAULT_WIDGET,
  buildWidgetUrl, resolveWidgetLook, sanitizeChatView,
} from "../lib/widget";
...
export default function DesktopApp() {
  ...
  const [chatViewRaw, setChatViewRaw] = usePersisted<ChatViewConfig>("chatView", DEFAULT_CHAT_VIEW);
  const chatView = useMemo(() => sanitizeChatView(chatViewRaw), [chatViewRaw]);
  ...
  const patchChatView = useCallback(
    (c: ChatViewConfig) => {
      setChatViewRaw(sanitizeChatView(c));
      markSaved();
    },
    [setChatViewRaw, markSaved]
  );
  ...
  /* Конфиг OBS-виджета уходит сразу при старте и при любом изменении —
     даже если вкладка «Виджет» сейчас закрыта. */
  useEffect(() => {
    if (!sp?.widgetConfig) return;
    sp.widgetConfig({ cfg: widgetCfg, look: resolveWidgetLook(widgetCfg) });
  }, [sp, widgetCfg]);
    ...
    >
      ...
      >
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <span className="truncate text-[14px] font-semibold tracking-tight">{APP_NAME}</span>
        </div>
                  ...
                  <div className="mx-auto max-w-2xl space-y-4">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 font-mono text-[10.5px]" style={{ color: "var(--dw-dim)" }}>
                            приложение для единой ленты стрима
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 grid gap-2 font-mono text-[11px]" style={{ color: "var(--dw-dim)" }}>
                        <div>версия программы: {APP_VERSION}</div>
                        <div>тег релиза: {APP_TAG}</div>
                        <div>редакция: portable x64 + установщик NSIS</div>
                        <div>режим: {desktop ? "desktop (Electron)" : "предпросмотр в браузере"}</div>
                        <div>каналов добавлено: {channels.length}</div>
                        <div>сейчас в эфире: {onlineCount}</div>
                        <div>лицензия: MIT</div>
                      </div>
                      <a
                        href={REPO_URL}
                        target="_blank"
