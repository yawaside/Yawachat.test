...
export function buildSpeechText(
  ...
  // Для озвучки смайлы не читаем — берём только текстовые части
  let rawForSpeech = msg.text;
  if (f.emoji) {
    rawForSpeech = stripPlatformEmotes(msg);
  }
  if (rawForSpeech.length > f.maxLen) return null;

  let text = rawForSpeech;
  ...
  if (f.emoji) {
    text = text
      ...
      .replace(/\[emote:\d+:[^\]]+\]/gi, "")
      .replace(/:[a-z0-9_\-]+:/gi, "")
      .replace(/\b(?:Kappa|PogChamp|LUL|KEKW|OMEGALUL|Pepega|monkaS|KEK|Pog)\b/gi, "");
  }
...
export function useSpeechEngine() {
  ...
  const spTts =
    typeof window !== "undefined"
      ? ((window as unknown as { sp?: { tts?: {
          speak: (p: { text: string; rate: number; volume: number; voice?: string; backend?: string; sileroSpeaker?: string }) => string;
          skip: () => void;
          stopAll: () => void;
  ...
  const [backend, setBackend] = useState<"sapi" | "silero">("sapi");
  const [sileroSpeaker, setSileroSpeaker] = useState("kseniya");
  ...
  const cfg = useRef({ enabled, paused, template, filters, rate, volume, voiceURI, obsTts, backend, sileroSpeaker });
  cfg.current = { enabled, paused, template, filters, rate, volume, voiceURI, obsTts, backend, sileroSpeaker };
    ...
    if (cfg.current.obsTts && typeof window !== "undefined" && (window as any).sp) {
      (window as any).sp.widgetConfig?.({
        ttsPlay: {
          ...
          backend: cfg.current.backend,
          sileroSpeaker: cfg.current.sileroSpeaker,
        },
      });
  ...
  return {
    ...
    obsTts, setObsTts,
    backend, setBackend,
    sileroSpeaker, setSileroSpeaker,
    voices,
    queueSize,
    now,
    skipped, setSkipped,
    spokenMinute,
    enqueue, skip, clearQueue, test,
  };
}
