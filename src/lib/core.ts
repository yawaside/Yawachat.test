...
export function buildSpeechText(
  ...
  if (f.emoji) text = text.replace(EMOJI_RE, "");
...
export function useSpeechEngine() {
  ...
  const cfg = useRef({ enabled, paused, template, filters, rate, volume, voiceURI });
  cfg.current = { enabled, paused, template, filters, rate, volume, voiceURI };
  ...
  const process = useCallback(() => {
    ...
    // Если включена озвучка через OBS, шлём событие в виджет. Если десктоп — шлём, иначе само произнесётся ниже.
    if ((cfg.current as any).obsTts) {
      if (typeof window !== "undefined" && (window as any).sp) {
    ...
    if (spTts && !(cfg.current as any).obsTts) {
