...
export function useSpeechEngine() {
    ...
    typeof window !== "undefined"
      ? ((window as unknown as { sp?: { tts?: {
          ...
          onEnd: (cb: (id: string) => void) => void;
           onAudio?: (cb: (p: { id: string; audioBase64: string }) => void) => void;
           onError?: (cb: (p: { id: string; error: string }) => void) => void;
        } } }).sp?.tts ?? null)
      : null;
  ...
  const process = useCallback(() => {
    ...
    /* ---------- Встроенные голоса Silero (voiceURI = "silero:<speaker>") ---------- */
    if (cfg.current.voiceURI && cfg.current.voiceURI.startsWith("silero:")) {
      if (typeof window === "undefined" || !(window as any).sp?.silero) {
      ...
      const speaker = cfg.current.voiceURI.slice("silero:".length);
      const spBridge = (window as any).sp;
      const finishSilero = () => {
      ...
      spBridge.silero
        .synthesize({
          ...
          volume: cfg.current.obsTts ? 1 : cfg.current.volume,
        })
        .then((r: { ok: boolean; empty?: boolean; wavBase64?: string; error?: string }) => {
          if (r?.ok && r.wavBase64) {
            // Если включена озвучка через OBS — WAV уходит в Browser Source,
            // где OBS управляет громкостью («Управлять аудио через OBS»).
            if (cfg.current.obsTts) {
              try {
                spBridge.widgetConfig?.({ ttsAudio: { id: item.id, audioBase64: r.wavBase64 } });
              } catch { /* noop */ }
              // Не проигрываем вторую копию в окне приложения: источник Browser
              // в OBS должен быть единственным владельцем этой аудиофразы.
              window.setTimeout(finishSilero, Math.max(1200, Math.min(12000, item.text.length * 75)));
              return;
            }
            const audio = new Audio("data:audio/wav;base64," + r.wavBase64);
            audio.onended = finishSilero;
            audio.onerror = finishSilero;
            audio.play().catch(finishSilero);
            return;
          }
          finishSilero();
        })
  ...
  // Legacy IPC path: a Silero WAV can be returned through tts:speak.
  useEffect(() => {
    if (!spTts?.onAudio) return;
    spTts.onAudio(({ id, audioBase64 }) => {
      if (pendingBridge.current !== id || !audioBase64) return;
  ...
  const preview = useCallback(
    async (voice: string, phrase?: string) => {
      ...
      // Silero — синтез через встроенный воркер
      if (voice && voice.startsWith("silero:") && typeof window !== "undefined" && (window as any).sp?.silero) {
        const speaker = voice.slice("silero:".length);
        try {
          const r = await (window as any).sp.silero.synthesize({
            text,
            speaker,
            rate: cfg.current.rate,
            volume: cfg.current.volume,
          ...
          return { ok: false, error: r?.error || "Silero не вернул аудио" };
        } catch (e: unknown) {
