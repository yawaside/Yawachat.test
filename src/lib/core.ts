...
export function useSpeechEngine() {
  ...
  const mapLang = (rawLang: string, name: string) => {
    const s = String(rawLang || "").trim();
    const low = s.toLowerCase();
    if (low.startsWith("ru")) return "ru-RU";
    if (low === "419" || low === "0x419" || low === "1049") return "ru-RU";
    if (low === "409" || low === "0x409" || low === "1033") return "en-US";
    if (/^[0-9a-f]{3}$/i.test(s)) {
      const dec = parseInt(s, 16);
      if (dec === 0x419) return "ru-RU";
      if (dec === 0x409) return "en-US";
    }
    if (!s && /ru|рус|silero|irina|pavel|ирина|павел|aidar|baya|kseniya|xenia|eugene/i.test(name)) return "ru-RU";
    return s || "en-US";
  };

  const parseVoiceRows = (rows: string[]) => {
    const parsed = rows.map((raw) => {
      const parts = String(raw).split("\u0001");
      const name = (parts[0] || "").trim();
      const langRaw = (parts[1] || "").trim();
      const lang = mapLang(langRaw, name);
      return { name, lang, langRaw };
    });
    const vs = parsed.map(
      (v) =>
        ({
    ...
    setVoices(vs);
    const ru = parsed.find((v) => v.lang.toLowerCase().startsWith("ru"));
    setVoiceURI((cur) => cur || ru?.name || parsed[0]?.name || "");
    return vs;
  };

  const reloadVoices = useCallback(() => {
    if (spTts) {
      spTts.voices().then((rows) => {
        if (rows.length) parseVoiceRows(rows);
      });
    } else {
      // Браузерный режим — перечитать Web Speech API
  ...
  return {
    ...
    skipped, setSkipped,
    spokenMinute,
    enqueue, skip, clearQueue, test, reloadVoices,
  };
}
