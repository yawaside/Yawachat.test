...
/** Разбирает текст на слова и смайлы. */
export function parseEmotes(text: string): EmoteToken[] {
  if (!text || cache.size === 0) return [{ type: "text", value: text }];
  const out: EmoteToken[] = [];
  let buf = "";
  for (const word of text.split(" ")) {
    const url = cache.get(word);
    if (url) {
      if (buf) {
        out.push({ type: "text", value: buf });
        buf = "";
      }
      out.push({ type: "emote", value: word, url });
      buf = " ";
    } else {
      buf += (buf && buf !== " " ? " " : buf === " " ? "" : "") + word + " ";
    }
  }
  if (buf.trim()) out.push({ type: "text", value: buf.replace(/\s+$/, "") });
