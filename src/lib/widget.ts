...
/** Нормализация настроек ленты из settings.json (старые конфиги без effect). */
export function sanitizeChatView(raw: unknown): ChatViewConfig {
  const src = (raw ?? {}) as Partial<ChatViewConfig>;
  const effect = (src.messageEffect as ChatViewConfig["messageEffect"]) || "slide-up";
  const allowed: ChatViewConfig["messageEffect"][] = [
    "none", "fade", "slide-up", "slide-left", "scale", "pop", "bounce",
  ];
  return {
    ...DEFAULT_CHAT_VIEW,
    ...src,
    messageEffect: allowed.includes(effect) ? effect : "slide-up",
    effectDuration: Math.max(0.1, Math.min(1.2, Number(src.effectDuration) || 0.34)),
    fontSize: Math.max(11, Math.min(28, Number(src.fontSize) || 15)),
    rowGap: Math.max(0, Math.min(24, Number(src.rowGap) || 6)),
    radius: Math.max(0, Math.min(28, Number(src.radius) || 16)),
  };
}
