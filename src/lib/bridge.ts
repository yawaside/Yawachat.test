...
/** Состояние встроенного движка Silero TTS. */
...
export interface SpBridge {
  ...
  /** Встроенные голоса Silero (v5_5_ru, русский) — без сторонних установок. */
  silero?: {
    status: () => Promise<SileroStatus>;
    install: () => Promise<{ ok: boolean; status: SileroStatus } | boolean>;
    synthesize: (p: {
      text: string;
      speaker: string;
      rate?: number;
      volume?: number;
    }) => Promise<{ ok: boolean; empty?: boolean; wavBase64?: string; error?: string }>;
    onStatus?: (cb: (s: SileroStatus) => void) => void;
  };
  overlay: {
