...
const FALLBACK_URL = MANUAL_DIRECT_URL;
...
class SileroTts {
  ...
  async install() {
    ...
    try {
      if (!this.isInstalled()) {
        // Сначала используем прямую рабочую ссылку, которую вы уже опубликовали.
        this.url = MANUAL_DIRECT_URL;
        this.tag = "1.0.20260829";
        try {
          await this.download(this.url);
        } catch (directErr) {
          // Если релиз позже обновится — ищем самый свежий через API GitHub.
