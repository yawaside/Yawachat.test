...
class SileroTts {
  ...
  async install() {
    ...
    try {
      if (!this.isInstalled()) {
        // Сначала проверяем, что есть хотя бы интернет
        try {
          await this._checkInternet();
        } catch (e) {
          this.lastError = e.message;
          console.error("[silero] проверка сети:", e.message);
          return false;
        }
        try {
          await this.download();
          ...
          this.lastError = e.message;
          console.error("[silero] download:", e.message);
          return false;
        }
        ...
        try {
          ...
          this.lastError = e.message;
          console.error("[silero] extract:", e.message);
          return false;
        }
      }
      this.lastError = null;
  ...
  /** Проверяет, что до GitHub можно достучаться — отдельная явная ошибка. */
  _checkInternet() {
    return new Promise((resolve, reject) => {
      const req = https.get(
        this.url,
        { method: "HEAD", timeout: 8000, headers: { "user-agent": "YawaChatHub-silero-worker" } },
        (res) => {
          // 302 на релизы — норма, но 5xx уже не норма
          res.resume();
          if (res.statusCode && res.statusCode >= 500) {
            reject(new Error(`Сервер отвечает ${res.statusCode}. GitHub недоступен.`));
          } else {
            resolve();
          }
        }
      ...
      req.on("timeout", () => {
        ...
        reject(new Error("Таймаут 8с: нет связи с github.com"));
      });
      req.on("error", (e) => reject(new Error(`Сеть: ${e.code || e.message}`)));
    });
  }
