...
// Совместимые имена используются ниже в статусе и fallback-логике.
const FALLBACK_TAG = "1.0.20260829";
const FALLBACK_URL = KNOWN_URL;
...
class SileroTts {
  ...
  get root() {
    // Поддерживаем ZIP с файлами в корне и ZIP с дополнительной папкой python/.
    const candidates = [
      this.dir,
      path.join(this.dir, "python"),
      path.join(this.dir, "silero-worker"),
    ];
    return candidates.find((p) => fs.existsSync(path.join(p, "python.exe"))) || this.dir;
  }

  setBaseDir(baseDir) {
    // Для NSIS/установленной версии папка приложения может быть Program Files
    // и недоступна для записи. Вызов делается после app.ready из main.js.
    this.dir = path.join(baseDir, "silero-worker");
  }

  get pythonExe() {
    return path.join(this.root, "python.exe");
  }

  get workerScript() {
    return path.join(this.root, "silero_worker.py");
  }

  get modelFile() {
    return path.join(this.root, "v5_5_ru.pt");
  }

  get manifestFile() {
    return path.join(this.root, "silero-worker.json");
  }

  isInstalled() {
    return (
      fs.existsSync(this.pythonExe) &&
      fs.existsSync(this.workerScript) &&
      fs.existsSync(this.modelFile) &&
      fs.existsSync(path.join(this.root, "python311.dll")) &&
      fs.existsSync(path.join(this.root, "python311.zip")) &&
      (
        fs.existsSync(path.join(this.root, "silero-worker.json")) ||
        fs.existsSync(path.join(this.root, "Lib", "site-packages", "torch", "__init__.py")) ||
        fs.existsSync(path.join(this.root, "torch", "__init__.py"))
      )
    );
  }
  ...
  async install() {
    ...
    try {
      ...
      // Загружаем модель сразу после распаковки: статус установки становится
      // рабочим, а пользователь сразу видит список внутренних голосов.
      this.speakers = ["aidar", "baya", "kseniya", "xenia", "eugene"];
      // Не блокируем кнопку установки на загрузке PyTorch-модели. Воркер
      // запускается в фоне, а первый preview/чат дождётся его готовности.
      this.ensureRunning().then(() => this.notifyStatus()).catch(() => this.notifyStatus());
      return true;
    } catch (e) {
  ...
  ensureRunning() {
    ...
    let child;
    try {
      child = spawn(this.pythonExe, [this.workerScript], {
        cwd: this.root,
        windowsHide: true,
