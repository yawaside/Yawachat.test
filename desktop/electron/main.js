...
const { SileroEngine } = require("./silero");
const { closeNet } = require("./net");
const { ensureGlobal: ensureEmotes, enrich: enrichEmotes } = require("./emotes");
...
ipcMain.on("widget:config", (_e, payload) => {
  if (payload && payload.ttsPlay && widgetServer && widgetServer.sendConfig) {
    const backend = payload.ttsPlay.backend || settings.tts?.backend || "sapi";
    const engine = backend === "silero" ? silero : tts;
    const synthesize = engine.synthesizeWavBase64
      ? engine.synthesizeWavBase64.bind(engine)
      : tts.synthesizeWavBase64.bind(tts);

    synthesize({
      text: payload.ttsPlay.text,
      rate: payload.ttsPlay.rate,
      volume: payload.ttsPlay.volume,
      voice: payload.ttsPlay.voice,
      speaker: payload.ttsPlay.sileroSpeaker || settings.tts?.sileroSpeaker || "kseniya",
    }).then((audioBase64) => {
      if (audioBase64) {
        widgetServer.sendConfig({ ttsAudio: { id: payload.ttsPlay.id, audioBase64 } });
      } else {
        // Fallback: пусть виджет сам попробует Web Speech API
...
silero.onEnd = (id) => broadcast("sp:tts-end", id);

function getTtsEngine(payload) {
  const backend = payload?.backend || settings.tts?.backend || "sapi";
  return backend === "silero" ? silero : tts;
}

ipcMain.on("tts:speak", (_e, payload) => {
  const engine = getTtsEngine(payload);
  // Silero ожидает speaker отдельно
  if (engine === silero) {
    engine.speak({
      id: payload.id,
      text: payload.text,
      speaker: payload.sileroSpeaker || settings.tts?.sileroSpeaker || "kseniya",
      rate: payload.rate,
...
ipcMain.on("tts:skip", () => { tts.skip(); silero.skip(); });
ipcMain.on("tts:stopAll", () => { tts.stopAll(); silero.stopAll(); });
ipcMain.handle("tts:voices", async () => {
  const [sapiVoices, sileroVoices] = await Promise.all([tts.voices(), silero.voices()]);
  // Префикс для Silero, чтобы различать в UI
  const combined = [...sapiVoices, ...sileroVoices.map((v) => `silero:${v}`)];
  return combined;
});
ipcMain.handle("tts:silero-check", async () => {
  return { available: await silero.checkAvailability(), speakers: silero._voices || [] };
});
