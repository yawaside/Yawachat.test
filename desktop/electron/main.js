...
ipcMain.on("widget:test", (_e, msg) => {
  if (widgetServer && msg && msg.text) widgetServer.broadcast(enrichEmotes(msg));
});
...
app.whenReady().then(async () => {
  ...
  createMainWindow();
  createTray();
  // глобальные смайлы (7TV/BTTV) — в фоне, не блокируют UI
  ensureEmotes().catch(() => {});
  ...
  connectors = new ConnectorManager({
    ...
    onChat: (m) => {
      emit("sp:chat", m);
      // реальные сообщения чата + смайлы → OBS-виджет
      if (widgetServer && m && !m.sys) {
        try { widgetServer.broadcast(enrichEmotes(m)); } catch { /* noop */ }
      }
    },
