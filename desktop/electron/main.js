...
app.whenReady().then(async () => {
  ...
  // коннекторы стартуют только после того, как интерфейс загрузился и готов принимать события
  ...
  if (mainWin) {
    mainWin.webContents.once("did-finish-load", () => setTimeout(startConnectors, 400));
    // подстраховка, если событие не пришло
    setTimeout(startConnectors, 4000);
  } else {
    startConnectors();
  }
