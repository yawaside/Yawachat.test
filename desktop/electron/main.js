...
silero.onStatus = (status) => {
  if (mainWin && !mainWin.isDestroyed()) mainWin.webContents.send("silero:status", status);
};
...
ipcMain.on("tts:speak", async (_e, payload) => {
  // Legacy/preview path: explicitly route Silero identifiers to the internal worker.
  if (String(payload?.voice || "").startsWith("silero:")) {
    const result = await silero.synthesize({
      text: payload.text,
      speaker: String(payload.voice).slice("silero:".length),
      rate: payload.rate,
      volume: payload.volume,
    });
    if (result.ok && result.wavBase64) {
      broadcast("sp:tts-audio", { id: payload.id, audioBase64: result.wavBase64 });
    } else {
      broadcast("sp:tts-error", { id: payload.id, error: result.error || "Silero не вернул аудио" });
      broadcast("sp:tts-end", payload.id);
    }
    return;
  }
  tts.speak(payload);
...
ipcMain.handle("silero:install", async () => {
  const ok = await silero.install();
  if (mainWin && !mainWin.isDestroyed()) {
    mainWin.webContents.send("silero:status", silero.status());
  }
  return { ok, status: silero.status() };
});
...
app.whenReady().then(async () => {
  ...
  if (!process.env.PORTABLE_EXECUTABLE_DIR) {
    // Установщик обычно ставит exe в Program Files. Модели Silero должны
    // храниться в доступной для записи папке пользователя, не требуя admin.
    silero.setBaseDir(path.join(app.getPath("userData"), "runtime"));
  }
  registerHotkeys();
  createMainWindow();
  createTray();
