...
contextBridge.exposeInMainWorld("sp", {
  ...
  silero: {
    status: () => ipcRenderer.invoke("silero:status"),
    install: () => ipcRenderer.invoke("silero:install"),
    synthesize: (p) => ipcRenderer.invoke("silero:synthesize", p),
    onStatus: (cb) => ipcRenderer.on("silero:status", (_e, s) => cb(s)),
  },
  ...
  tts: {
    ...
    onError: (cb) => ipcRenderer.on("sp:tts-error", (_e, payload) => cb(payload)),
  },
