...
contextBridge.exposeInMainWorld("sp", {
  ...
  /* SAPI озвучка */
  tts: {
    speak: ({ text, rate, volume, voice, backend, sileroSpeaker }) => {
      const id = `t${Date.now()}${Math.random().toString(36).slice(2, 7)}`;
      ipcRenderer.send("tts:speak", { id, text, rate, volume, voice, backend, sileroSpeaker });
      return id;
    },
    skip: () => ipcRenderer.send("tts:skip"),
    stopAll: () => ipcRenderer.send("tts:stopAll"),
    voices: () => ipcRenderer.invoke("tts:voices"),
    sileroCheck: () => ipcRenderer.invoke("tts:silero-check"),
