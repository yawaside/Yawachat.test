...
function New-PathsManifest {
    # Манифест путей для native/CMakeLists.txt. Все подстановки вычисляются
    # ЗАРАНЕЕ: интерполяция "${(...)}" внутри here-string в PowerShell — это
    # имя переменной (пустая строка), а не выражение — на этом CI уже падал
    # (list(APPEND CMAKE_PREFIX_PATH "") → find_package(Qt6) не находил пакет).
    param(
        [string]$ObsSourceDir,
        [string]$ImportLibDir,
        [string]$QtDir,
...
set(YAWAMETRICS_OBS_LIBOBS_INCLUDE "$libobsFwd")
set(YAWAMETRICS_OBS_FRONTEND_INCLUDE "$frontendFwd")
set(YAWAMETRICS_OBS_CONFIG_INCLUDE "$libobsFwd")
set(YAWAMETRICS_OBS_IMPORT_LIB_DIR "$importFwd")
set(YAWAMETRICS_QT6_PREFIX "$qtFwd")

list(APPEND CMAKE_PREFIX_PATH "$qtFwd")
"@
    [System.IO.File]::WriteAllText($OutPath, $manifest.Replace("`r`n", "`n"))

    # Самопроверка: манифест обязан содержать пути к Qt6 и импорт-библиотекам.
    $written = [System.IO.File]::ReadAllText($OutPath)
    foreach ($expected in @($qtFwd, $libobsFwd, $frontendFwd, $importFwd)) {
        if (-not $written.Contains($expected)) {
            throw "Paths manifest at $OutPath is missing expected path: $expected"
        }
    }
    if ($written -match 'CMAKE_PREFIX_PATH\s+""') {
        throw "Paths manifest at $OutPath has an empty CMAKE_PREFIX_PATH entry"
    }
}
New-PathsManifest `
    -ObsSourceDir $ObsSourceDir `
    -ImportLibDir $ImportLibDir `
    -QtDir $QtDir `
    -OutPath (Join-Path $DepsDir "obs-include-dirs.cmake")
